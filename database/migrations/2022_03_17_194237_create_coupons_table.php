...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('coupons', function (Blueprint $table) {
            ...
            $table->enum('discount_type',['f','p'])->default('f')->comment('f=>fixed,p=>percentage');
            ...
            $table->smallInteger('max_usage_per_user')->nullable();
            $table->tinyInteger('website_percentage')->default('100');
            $table->timestamp('start_date')->nullable();
            $table->timestamp('end_date')->nullable();
            $table->tinyInteger('status')->default(0)->comment('0=>not valid,1=>valid');
            $table->timestamps();
        });
    }
