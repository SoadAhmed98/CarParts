          ...
          <tbody>
              @forelse ($regions as $region)
                <tr>
                    ...
                    <td>{{$region->radius}}</td>
                    <td><label class="badge badge-{{$region->status == 1 ? 'success' : 'danger' }}">{{$region->status == 1 ? 'متاح التوصيل' : 'غير متاح التوصيل'}} </label></td>
                    <td>{{$region->created_at}}</td>
                    <td>{{$region->updated_at}}</td>
                    <td>
                        <a href="{{route('regions.edit',['region'=> $region->id])}}" class="btn btn-outline-primary btn-sm">تعديل</a>
                        <form method="post" action="{{route('regions.destroy',['region'=>$region->id])}}" class="d-inline-block">
                            @csrf
                            @method('delete')
              ...
              @empty
                  <tr>
                      <td  colspan="11" class="alert alert-warning font-weight-bold text-center">لا يوجد مناطق حاليا</td>
                  </tr>
              @endforelse
